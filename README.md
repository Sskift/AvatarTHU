...
个人课程助手：每天扫描清华网络学堂，把未读公告发到本人飞书并标为已读；按课程下载课件、作业描述和附件，用一个 Claude Code 会话完成每份作业，把产物集中嵌入飞书云文档，会话只发送交互卡片，由本人决定提交或修改。
...
4. **审核**：每版生成本人拥有的飞书云文档，固定分为“作业描述”“完成情况与关键结果”“完整产物”“审阅与操作”四部分。原题附件、报告、源码包、程序、其他交付文件与自查记录都嵌入文档，可预览或下载。会话只发送一张文档入口与操作卡片，不单独发送任何产物或提交包。点击“审阅完成，提交本版”才提交。
5. **修改**：可以在审阅文档里圈选文字、图片或源码写批注，写完后点击卡片“按文档批注修改”。只读取本人未解决的批注，收集完整分页和可用的原文位置；点击后才开始下一版，不会在写到一半时启动。也可以直接填写卡片意见或回复卡片。下一版逐项响应意见，旧版按钮立即失效。直接编辑审阅文档不改变提交文件。
6. **提交**：校验本人身份、当前消息、版本、随机标识和文件哈希，重新检查学校的待交状态、截止时间、题目和附件版本。收到学校成功回执才显示提交成功。若响应丢失，进入 `submission_unknown`，需本人在网页核对，不会自动重传。
...
同步或执行失败会记录日志并通知本人，15 分钟后重试；也可运行 `./run-now.sh`。云文档发布回执会持久保存，卡片发送失败重试时复用已发布文档。卡片回调和回复通过 `lark-cli event consume` 长连接接收，不需要公网回调服务器；需启用 `card.action.trigger` 和 `im.message.receive_v1`。
