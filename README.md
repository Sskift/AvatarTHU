...
4. **审核**：每版生成本人拥有的飞书审阅文档，直接展示题目、PDF 原页、产物附图、正文/源码和执行自查。卡片展示预览图，主按钮为“打开审阅文档”；完整提交包放在文档中，小于等于 20 MB 时也作为单个聊天附件发送。点击“审阅完成，提交本版”才提交。
...
审阅文档从本版冻结文件生成，展示程序不执行作业代码。每版最多直接渲染 30 页 PDF、16 张独立附图，源码有字符上限，超出部分通过完整包查看；正文会标注预览范围。GUI 示意图与真实截图分开说明。原生文档支持手机阅读和批注；当前不提供在飞书里运行桌面程序的功能。
